...
function codificationItem($mdp)
{
    global $pdo;
    $req = $pdo->prepare("SELECT * FROM etudiant WHERE mdp = ? ");
...
function EditeEtudiantById($prenom, $nom, $email, $genre, $codifier, $id)
{
    global $pdo;
    $req = $pdo->prepare("UPDATE etudiant SET prenom = :prenom, nom = :nom, email = :email, genre = :genre codifier = :codifier WHERE id= :id");
...
function SiteEtPavillonCorrespond($item)
{
